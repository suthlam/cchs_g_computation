...
APvar.threshold.3 <- log(5)       # revised WHO guideline value
APvar.threshold.4 <- log(4)       # 1 ug/m3 below revised WHO guideline
APvar.threshold.5 <- log(3)       # 2ug/m3 below revised WHO guideline
APvar.threshold.6 <- log(2)       # 3 ug/m3 below revised WHO guideline
APvar.threshold.7 <- log(1.8)     # all the way down to our presumed background conc ug/m3
APvar.threshold.8 <- log(8.8*0.8) # 20% below Canadian national standards
...
interventions <- list(list(c(threshold, -Inf, APvar.threshold.1)), # all time-points with levels above CA standard set to 12
                      ...
                      list(c(threshold, -Inf, APvar.threshold.3)), # all time-points with levels above theoretical minimum risk level of GDB set to 5
                      list(c(threshold, -Inf, APvar.threshold.4)), # all time-points with levels above theoretical minimum risk level of GDB set to 4
                      list(c(threshold, -Inf, APvar.threshold.5)), # all time-points with levels above theoretical minimum risk level of GDB set to 3
                      list(c(threshold, -Inf, APvar.threshold.6)), # all time-points with levels above theoretical minimum risk level of GDB set to 2
                      list(c(threshold, -Inf, APvar.threshold.7)), # all time-points with levels above theoretical minimum risk level of GDB set to 1.8
                      list(c(threshold, -Inf, APvar.threshold.8)), # all time-points with levels above theoretical minimum risk level of GDB set to 80% below Canadian standards
                      list(c(relative_reduction, 0.95, APvar.threshold.7)), # all time-points set to 95% of its original value
                      list(c(relative_reduction, 0.9, APvar.threshold.7)), # all time-points set to 90% of its original value
                      list(c(relative_reduction, 0.8, APvar.threshold.7))  # all time-points set to 80% of its original value
)

int_descript <- c("Lower or equal to 12", "Lower or equal to 8.8", 
                  "Lower or equal to 5", "Lower or equal to 4",
                  "Lower or equal to 3", "Lower or equal to 2",
                  "Lower or equal to 1.8",
                  "Lower or equal to 80% of 8.8",                
                  "Reduced to 95% of original",
                  "Reduced to 90% of original",
                  "Reduced to 80% of original"
)
##################
...
gform_1 <- gformula_survival(
  ...
  intvars = list(expsr, expsr, expsr, expsr, expsr, expsr, expsr, expsr, expsr, expsr, expsr),
...
range <- 1:200 # e.g., 1:100 and 101:200, please use different range for each job so that the seed is different for each bootstrap
...
foo <- foreach(i = range,
               ...
               .packages=c("gfoRmula", "data.table", "Hmisc")) %dopar% {
                 ...
                 new[, AGE3 := NULL]
                 new[, AGE4 := NULL]
                 rcs <- data.frame(rcspline.eval(new$AGE_new, nk=5, inclx = TRUE))
                 ...
                 saveRDS(gform_2$result, file.path(outdir_bt, paste0("Cycle", wave, ".gformula_results.", batch, i, ".rds")))
               }
stopCluster(cl)
rm(foo)
...
risks_bs <- NULL
for (i in range) {
  risk_ <- readRDS(file.path(outdir_bt, paste0("Cycle", wave, ".gformula_results.", batch, i, ".rds")))
...
if (nrow(temp) > 0) {
  ...
  if (sum(unique(temp$k) != 0) > 0 ) {
    ...
    write.csv(temp, file.path(outdir_bt, paste0("Cycle.", wave,"Error_duplicated_results.", batch, ".csv")), row.names = FALSE) ## Edit me if want to save error message somewhere else
  } else {
...
l_risk_bs.hc <- risks_bs %>% group_by(Intervention, k) %>% 
...
l_risk_bs.hc <- as.data.frame(l_risk_bs.hc) 
##################